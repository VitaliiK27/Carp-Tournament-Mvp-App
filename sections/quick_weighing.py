...
ADD_CATCH_SUCCESS_KEY = "quick_weighing_add_success"
...
def render_quick_weighing_page(active_meta: dict, active_tournament_id: int):
    ...
    if st.button("Зберегти зважування", key="save_new_catch", use_container_width=True):
        if weight is None:
            ...
            if period is None:
                ...
                st.session_state[ADD_CATCH_SUCCESS_KEY] = (
                    f"Зважування збережено: {selected_team_name}, {float(weight):.3f} кг, період {period}"
                )
                st.rerun()

    add_success_message = st.session_state.pop(ADD_CATCH_SUCCESS_KEY, None)
    ...
    with st.expander("✏️ Редагувати зважування", expanded=False):
